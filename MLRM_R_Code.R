...
Path<-"C:/Ganesha_Accenture/Ganesha_IVY/Gan_B01/03Data"
setwd(Path)
getwd()
...
#Imputing the values with the median value of the series
A1[is.na(A1$b),"b"]=median(A1$b,na.rm=T)
A1
...
FinalModel=lm(clv~	Coverage+	I(Education=="College")+	I(EmploymentStatus=="Medical Leave")+	 Monthly.Premium.Auto + Number.of.Open.Complaints+	Number.of.Policies +	Renew.Offer.Type+	I(Vehicle.Class=="SUV"), data=original.data)
...
#Calculating MAPE
attach(test_data)
(sum((abs(clv-pred))/clv))/nrow(test_data)
